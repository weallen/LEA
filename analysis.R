...
#  d <- MEDIPS.getPositions(data=d, pattern="C")
...
reduceMedipsResolution <- function(medips.obj, window.size) {
  res.len <- ceiling(genome_raw(medips.obj) / window.size)
  for (chr in medips@chr_names) {
  }
  in.len <- length(genome_raw(medips.obj))
  medips.obj@genome_chr
}

# fits a poisson GLM to each window
diffMeth <- function(medips1, medips2, window.size=1000) {
  require(MASS)
